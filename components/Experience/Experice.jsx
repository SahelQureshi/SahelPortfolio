...
const Experience = () => {
  ...
  useEffect(() => {
    ...
    if (headerRef.current) {
      headerRef.current.classList.add('experience-header', 'animate-from-bottom');
      observer.observe(headerRef.current);
    }

    if (timelineRef.current) {
      timelineRef.current.classList.add('experience-timeline', 'animate-from-rotate');
    ...
    cardsRef.current.forEach((card, index) => {
      if (card) {
        ...
        const animationClass = index % 2 === 0 ? 'animate-from-right' : 'animate-from-left';
        card.classList.add('experience-card', animationClass);
